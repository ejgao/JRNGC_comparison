...
# add any functions defined here
...
if "__main__" == __name__:
    ...
    for trial in range(args.current_start, trials):
        ...
        if args.data_type == "var":
            data_name = "num_nodes_{}/true_lag_{}/noise_scale_{}/seed_{}/".format(
                params["d"], params["true_lag"], params["noise_scale"], cur_seed
            )

        elif args.data_type == "lorenz":
            data_name = "num_nodes_{}/F_{}/seed_{}/".format(
                params["d"], params["f_lorenz"], cur_seed
            )
        elif args.data_type == "fmri":
            data_name = "num_nodes_{}/subject_{}/seed_{}/".format(
                params["d"], args.f_subject, cur_seed
            )
            ...
            data_name = "num_nodes_{}/{}/seed_{}/".format(
                params["d"], dream3_using_data, cur_seed
            )
        ...
        try:  # loading data
            ...
            if args.data_type == "var":
                ...
                np.save(data_dir + data_name + "_beta.npy", beta)  # save the beta

            elif args.data_type == "fmri":
                print("-----fmri----")
                x, x_eval, gc = fmri_net_sim(
                    d=num_nodes,
                    subject=args.f_subject,
                    t=args.f_t,
                    t_eval=args.f_t_eval,
                )
            elif args.data_type == "lorenz":
                print("generating data from scratch...")
                x, x_eval, gc = lorenz_96(
                    d=num_nodes,
                    t=args.lorenz_t,
                    t_eval=args.lorenz_t_eval,
                    f=params["f_lorenz"],
                    seed=cur_seed,
                )
                np.save(data_dir + data_name + "_x.npy", x)
                ...
                print("dream3-------")
                x, x_eval, gc = dream3_trajectories(
                    d=num_nodes, subject=args.dream3_subject
                )
        ...
        if args.jaco_param:
            # training
            model, it, a, b, c, best_loss_r, mean_pred_loss_r, eval_loss_r = (
                JRNGC.from_train(
                    ...
                    jacobian_lam=args.jaco_param,
                    struct_loss_choice=params["struct_loss_choice"],
                    JFn=params["JFn"],
                    x=x,
                    x_eval=x_eval,
                    lr=params["lr"],
            ...
            model, it, a, b, c, best_loss_r, mean_pred_loss_r, eval_loss_r = (
                JRNGC.from_train(
                    ...
                    hidden=params["hidden"],
                    dropout=params["dropout"],
                    jacobian_lam=params["jacobian_lam"],
                    struct_loss_choice=params["struct_loss_choice"],
                    JFn=params["JFn"],
                    x=x,
                    x_eval=x_eval,
        ...
        if print_params:
            ...
            with open(
                "./result/{}/model_params_result_{}_{}.txt".format(
                    args.model_type, args.data_type, num_nodes
                ),
                "a",
            ) as f:
                ...
                if args.model_type == "JRNGC":
                    f.write(
                        "the JRNGC {}  params:{}\n".format(
                            params["struct_loss_choice"], num_params
                        )
                    )
        ...
        key_i = 0
        result_filename = "./result/{}/data_{}/seed_{}.txt".format(
            args.model_type, args.data_type, cur_seed
        )
